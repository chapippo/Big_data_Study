...
data = tree.find('./data').findall('item') #item들만 모은게 data

xml_arr = []    # 배열 선언
xml_dictionary = {} #자바스크립트의 객체, 파이썬에선 이걸 딕셔너리라고 함
#자바스크립트의 객체와 마찬가지로 속성 추가하고 싶으면
#그냥 새로 써주면 알아서 속성 추가됨. (xml_dictionary에 원래 {}처럼 아무것도 없었는데 밑에서 '연번' 추가함)
for item in data: # data: <item>태그의 모음(item 하나하나를 의미 !!!)
    xml_dictionary['연번']=item.find('./col[@name="연번"]').text #(.text(=태그안의 값들))이것의 text를 긁어옴
    try:
        xml_dictionary['서명']=item.find('./col[@name="서명"]').text
    except:
        xml_dictionary['서명']='' #(간혹 '서명'이 없는 것들에) 비어 있는 값 넣음
    xml_dictionary['가격']=item.find('./col[@name="가격"]').text
    xml_arr.append(xml_dictionary)
    xml_dictionary={} #값 초기화 해주기
