function genLotto(startnum)
{
    ...
    //확실하게 '숫자'로 형변환 해주는 것
    //(숫자로 형변환 하지 않으니까 제대로 비교 못함. value값을 가져오는데 제대로 못해)
    ...
    for(let i=1; i<6; i++)
    {
        ...
        //indexOf : 해당 값이 배열 안에서 어디에 있는지 표시해줌
        //만약 배열이 [10,20,30]인데 indexOf(20)하면
        //숫자 1을 반환해줌. 왜냐면 [1]번째에 20이 있어서.

        //그리고 indexOf(40)이라고 적으면 40은 배열 안에 없으므로
        //-1을 반환, 즉 존재하지 않으면 -1
    ...
    //즉 콜백함수 @@@
    //sort 안에 들어가는 함수를 이용해서 배열 정렬
    //매개변수 a와 b는 배열 안에 있는 값
    //두 개의 값을 서로 비교해서 음수, 0, 양수
    //뭐가 나오냐에 따라서 정렬이 달라짐
    //a,b 대신 x,y등을 넣어도 됨
    lotto.sort(function(a,b) {return a-b})  //a,b를 매개변수로 해서 a-b 리턴
    return lotto    //배열을 리턴
}